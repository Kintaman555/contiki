...
#endif /* TSCH_QUEUE_MAX_NEIGHBOR_QUEUES */
...
/*---------------------------------------------------------------------------*/
// void update_from_net(struct neighbor_list *list, u16_t rtmetric)
// {
...
{ // in listen mode, node sends beacon after waking-up
  ...
  beacon.send_time = timesynch_time();

  NETSTACK_RADIO.send(&beacon, sizeof(beacon));
  timesynch_synch_delay(timesynch_time() - beacon.send_time);
}
/*---------------------------------------------------------------------------*/
static void
  ...
  case TYPE_DATA: {
    ...
    if(!is_broadcast){
      ...
      /* for me, so send ACK, then notify upper layer */
        ...
        ack.send_time = timesynch_time();
        NETSTACK_RADIO.send(&ack, sizeof(ack));
        //timesynch_synch_delay(timesynch_time() - ack.send_time);
