...
//printEdges();          
  ...
  var str = "Attenuated motes: ";
  for(i=0; i<shuffledIDs.length/2; i++) {
  	str = str + shuffledIDs[i] + ", ";
  }
  log.log(str + "\n"); 
...
//70min*60sec*1000ms
 ...
 logMsg = time + "\tID:" + mote.getID() + "\t" + msg + "\n";
    ...
    if(((time/1000) > step * timeSlice) && step < shuffledIDs.length/2) {
    	var id = shuffledIDs[step-2];
    	var dstRadio = sim.getMoteWithID(id).getInterfaces().getRadio();
    	...
    	for (i=0; i<edges.length; i++) {
            ...
            if (edge.superDest.radio == dstRadio) {
              ...
              edge.superDest.ratio *= 0.8;
              str = java.lang.String.format("%d [COOJA] %d -> %d, ratio: %.2f, was: %.2f\n", new Integer(time/60000000), new Integer(edge.source.getMote().getID()), new Integer(edge.superDest.radio.getMote().getID()), new Float(edge.superDest.ratio), new Float(oldRatio));
			  log.log(str);
            }
          }
