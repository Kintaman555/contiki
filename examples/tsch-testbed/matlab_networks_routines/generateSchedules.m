function [ slotframeTable, linkTable, parents, expectedLatency ] = generateSchedules( myNetwork, paths, sfHandle, chOffset, ebSfHandle, ebChOffset, ebPeriod, tsDuration, latencyGoal )
...
for i=2:length(paths)
    ...
    for j=length(paths{i}):-1:2
       ...
       dst = paths{i}(j-1);
       parents(src) = dst;
       etx = max(1, round(2*myNetwork(src, dst)));
